...
def generate_launch_description():
            ...
            actions=[
                Node(
                    ...
                    parameters=[
                        ...
                        {'obstacle_push_distance': 0.25}  # Increased
                    ]
                )
            ]
        )
            ...
            actions=[
                Node(
                    ...
                    parameters=[
                        ...
                        {'max_detection_range': 0.3},
                        {'min_cluster_size': 4}  # Increased for larger boxes
                    ]
                )
            ]
        )
    ...
    box_types = {
        'standard': {'size': '0.1 0.1 0.1', 'mass': 0.05, 'friction': 0.5},    # Increased from 0.03m
        'small': {'size': '0.08 0.08 0.08', 'mass': 0.03, 'friction': 0.4},   # Increased from 0.04m
        'flat': {'size': '0.06 0.1 0.08', 'mass': 0.04, 'friction': 0.45},    # Increased from 0.02x0.04x0.03
        'tall': {'size': '0.08 0.08 0.12', 'mass': 0.05, 'friction': 0.4}     # Increased from 0.04x0.04x0.02
    }

    # Define safe spawn area within maze walls
    safe_spawn_range_x = (-9, 9)
    safe_spawn_range_y = (-9, 9)
    wall_buffer = 0.5  # Increased to avoid collisions with larger boxes
    ...
    def generate_position(existing, min_dist=0.5):  # Increased for larger boxes
    ...
    def generate_stack_position(existing, min_dist=1.0):  # Increased for larger boxes
...
<sdf version="1.6">
  <model name="stackable_box">
    ...
    num_scattered_boxes = random.randint(15, 20)  # Reduced due to larger boxes
    for i in range(num_scattered_boxes):
        ...
        ld.add_action(spawn_stackable_box(
            ...
            box_type, color_name, x, y, 0.05, 0.1  # Increased spawn height
        ))

    # Create intentional stacks
    num_stacks = random.randint(3, 5)  # Reduced due to larger boxes
    box_counter = num_scattered_boxes + 1
        
    for stack_i in range(num_stacks):
        stack_x, stack_y = generate_stack_position(stack_positions)
        stack_height = random.randint(2, 3)  # Reduced for stability
            
        current_z = 0.05  # Increased start height for larger boxes
            
        for level in range(stack_height):
            color_name = random.choice(list(colors.keys()))
            box_type = random.choice(['standard', 'small', 'flat'])
                
            offset_x = random.uniform(-0.01, 0.01)  # Slightly increased offset
            ...
            ld.add_action(spawn_stackable_box(
                ...
                3.0 + stack_i*1.0 + level*0.2
            ))
                
            if box_type == 'flat':
                current_z += 0.08  # Increased for larger flat boxes
            elif box_type == 'small':
                current_z += 0.08  # Increased for larger small boxes
            else:  # standard
                current_z += 0.10  # Increased for larger standard boxes
